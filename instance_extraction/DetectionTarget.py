...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    positive_roi_bool = roi_iou_max >= config.MAP_IOU

    # Subsample ROIs. Aim for 33% positive
    # Positive ROIs
    if len(torch.nonzero(positive_roi_bool).size()) > 1:
        positive_indices = torch.nonzero(positive_roi_bool)[:, 0]  # x,1
    ...
    negative_roi_bool = roi_iou_max < config.MAP_IOU
